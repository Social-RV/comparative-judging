...
This module provides helper functions for encoding files, downloading images from URLs,
and preparing data for the decoy judge agent.
"""
...
from .agent import SessionFile, TargetImage
...
def create_session_file_from_url(url: str, filename: Optional[str] = None) -> SessionFile:
    """
    Create a SessionFile from a URL by downloading and encoding the content.
    ...
    Returns:
        SessionFile with the downloaded and encoded content
    """
    base64_content, mime_type = download_and_encode_file(url)
...
def create_target_from_url(target_id: str, description: str, image_url: str) -> TargetImage:
    """
    Create a TargetImage from an image URL by downloading and encoding it.
    ...
    Returns:
        TargetImage with the downloaded and encoded image
    """
...
def create_session_files_from_urls(urls: List[str]) -> List[SessionFile]:
    ...
    Args:
        urls: List of file URLs to download and encode
    ...
    for i, url in enumerate(urls):
        try:
            ...
            print(f"✅ Successfully downloaded: {url[:80]}...")
...
def create_targets_from_urls(target_data: List[dict]) -> List[TargetImage]:
    """
    Create a list of TargetImages from target data with URLs.
    ...
    for data in target_data:
        try:
            ...
            print(f"✅ Successfully downloaded target: {data['id']}")
        except Exception as e:
            print(f"❌ Failed to download target {data.get('id', 'unknown')}: {e}")
...
def create_test_data_from_urls(
    ...
    correct_target_data: dict,
    decoy_targets_data: List[dict]
) -> Tuple[List[SessionFile], TargetImage, List[TargetImage]]:
    """
